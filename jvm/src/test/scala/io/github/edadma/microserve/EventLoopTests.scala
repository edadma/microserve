...
/** JVM-internal tests for the event loop. The loop itself is `private[microserve]`
  * so these tests must live in this package on this platform; they are not
  * cross-platform because Node and libuv each have their own loop with their
  * own semantics — those are exercised through the public Server API in the
